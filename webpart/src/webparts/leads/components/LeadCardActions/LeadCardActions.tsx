...
export class LeadCardActions extends React.Component<ILeadCardActionsProps, {}> {
  private _getPercentCompleteIconName(percentComplete: number): string {
  ...
  private _getChangeIconName(change: number): string {
    if (change >= 0) {
      return 'stockUp';
    }
    else {
    ...
    return (
      <div className={css('ms-DocumentCardActions', styles.actions)}>
        ...
        <div className={styles.metrics}>
          <div className={css('ms-DocumentCardActions-views icon', styles.metric, styles.change, change >= 0 ? styles.up : styles.down, this._getChangeIconName(change))}>
            {(change >= 0 ? '+' : '')}{change}%
          </div>
          <div className={css('ms-DocumentCardActions-views icon', styles.metric, this._getPercentCompleteIconName(percentComplete))}>
            {percentComplete}%
          </div>
        </div>
      </div>
